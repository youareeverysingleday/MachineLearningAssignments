...
总之，邻域模型易于实现，基本上具有良好的可解释性。然而，**邻域方法无法对用户顺序签入行为中的底层属性进行建模**。因此，它们通常与其他模型结合使用。
...
值得注意的是，尽管统计学习模型的实现相对简单，**但这些方法通常要求每个用户有足够的签入数据进行模型训练，这是不现实的，因为用户的签入记录通常非常稀少**。除此之外，这类模型通常从用户移动数据或相应的社交网络中选择一组hand-crafted features的功能。这样一个手动特征工程过程通常不仅需要领域专家的繁琐工作，而且表现出较少的通用性[22,106]。
