...
使用给定地域内的GPS数据，图18显示了随GPS轨迹数变化的OPTICS聚类结果。![在改变GPS轨迹数量的情况下基于改变点簇的变化](../pictures/UnderstandingMobilityBasedonGPSDataFigure18.png "在改变GPS轨迹数量的情况下基于改变点簇的变化")因此，该区域内的聚类数不会随着GPS轨迹的增加而不断增加。**这证明了在给定区域内大多数人改变交通的地方数量是有限的，并且受到真实世界的约束**。这也为我们基于图的后处理的可行性提供了积极的支持（这为什么提供了支持？）。关于OPTICS算法，其结果取决于两个参数，核心距离（$CorDist$）和核心距离内的最小点（$MINPT$）。根据现实世界的常识和实验评估，我们发现当CorDist=25和minPts=5时，聚类的分布比基于其他参数的分布更有意义。
