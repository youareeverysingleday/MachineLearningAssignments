...
3. 参数点估计的两种方法。**注意：两种方法估计的结果有可能是不相同的。一般认为极大似然估计的效果比矩估计的效果要好一些**。
   ...
   4. 几种重要分布的矩估计
      1. $X\sim B(1,p)$
4. 极大似然估计方法不能对所有分布来使用。比如在对超几何分布和平均分布进行参数估计时，就不适用于极大似然估计。因为无法通过求导来求极值。可以采用其他的求极值的方法来对其进行解。
...
1. 幂律分布也叫长尾分布。
2. 密度函数为$y=cx^{-r}, both c,r >0 and are constants.$
3. 对密度函数取对数$\ln y = \ln c - r\ln x$。也即在双对数坐标下，幂律分布表现为一条斜率为幂指数的负数的直线，这一线性关系是判断给定的实例中随机变量是否满足幂律的依据。