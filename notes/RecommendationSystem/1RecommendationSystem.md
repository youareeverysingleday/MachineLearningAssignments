...
2. 推荐算法简介，以下分类主要是将数据源作为分类的。
   ...
   3. 基于协同过滤的推荐![协同过滤，重要的是看那个矩阵](../../pictures/RecommendationSystemCF.png "协同过滤，重要的是看那个矩阵")
      1. 基于**近邻的协同过滤**，这就是论文里面所说的neighborhood。
         1. 基于用户User-CF
            1. 基于用户喜欢的物品行为来对物品进行推荐。举例：u1、u2、u3共3个用户，i1、i2、i3、i4共4个物品；u1喜欢i1、i3、i4，u2喜欢i2，u3喜欢i1、i3。可以看出u2与u1、u2喜欢的物品不一样，而u3喜欢的物品u1都喜欢，也就是u1和u3向相似度很高，而且u1相比于u3还喜欢i4，这样就可以将i4推荐给u3。![基于用户的协同过滤](../../pictures/RecommendationSystemUserCF.png "基于用户的协同过滤")
         2. 基于物品Item-CF
            1. 基于物品被谁喜欢的行为来对物品进行推荐。如图所示，在这种情况下，从用户的角度并不好进行推荐。这个时候可以通过物品的角度来考虑。u1、u2、u3共3个用户，i1、i2、i3共3个物品；u1喜欢i1、i3，u2喜欢i1、i2、i3，u3喜欢i1。从物品的角度考虑，i1被u1、u2、u3喜欢，i2只被u2喜欢，i3被u1、u2喜欢。此时可以认为i1、i3更相似。所以可以考虑将i3推荐给u3。![基于物品的协同过滤](../../pictures/RecommendationSystemItemCF.png "基于物品的协同过滤")
      2. 基于模型的协同过滤，基于模型的分类。
         1. 奇异值分解SVD
         2. 潜在语义分析LSA
         3. 支持向量机SVM
   ...
   6. 协同过滤的缺点。所有缺点的来与都与历史数据本身的特点有关。：
      1. **历史数据本身稀疏。得到的行为交互信息是一个稀疏矩阵**（这里需要强调，在单纯的推荐系统中得到的行为数据也是稀疏的。因为用户不会对所有的物品都有交互行为，有交互行为的商品毕竟是极少数。对应于POI推荐，访问的地点相对于所有地点而言也是极少数）。稀疏矩阵上的计算有些明显的问题，包括可能少部分人的错误偏好会对推荐的准确度有很大的影响等等。
      2. 历史数据可能不准。比较依赖历史数据，推荐的效果依赖于用户历史偏好数据的多少和准确性。
      3. 如果没有历史数据，那么就面临冷启动问题。新系统刚上线的时候是没有历史数据的。基础的推荐可以使用统计推荐之一的热门物品推荐给用户。或者根据用户特征来提供推荐。所以在实际的应用中是将CB和CF结合起来一起使用的。
...
3. 基于内容的推荐与特征工程
   ...
   3. **将user个人信息的特征（基于喜好记录或是预设兴趣标签）和item的特征相匹配，就能得到user对item感兴趣的程度**。
      1. 在一些电影、音乐、图书的社交网站有很成功的应用，有些网站还请专业的人员对物品进行基因编码/打标签（PGC，专家生成内容，实际上就是人工打标签。与之对应的是UGC，称为用户生成内容，相当于是用户的行为数据，也就是用户对item打的标签）。
      2. 由于用算法对电影、音乐、图书打的标签并不理想，所以采用了人工打标签的方法来对item进行标注。对应于PGC称为专家生成内容，UGC称为用户生成内容。
   4. 相似度计算
      1. 相似度的评判可以用距离表示，一般常用的“余弦相似度”。之所以用余弦相似度有两个原因。
         |![余弦相似度](../../pictures/RecommendationSystemCosineSimilarity.png "余弦相似度")|
         |:--:|
         | *2.1* |
         1. 一个苛刻的用户A对Item的评分是一般都比较低，一个宽松的用户B对item的评分一般都比较高。比如A对电源的评分是3，而B对同一个电源的评分是5。这两个评分都说明在各自用户的评价中算是好的电影。这个时候用绝对值来评价的时候并不能真实反应电源的之间的相似度。
         2. 计算点积的时候是使用模长乘以余弦值来计算的。所以使用余弦值来作为相似度的度量单位。
      2. 欧式距离：$d(x,y) = \sqrt{\sum\limits_{i}(x_i - y_i)^2}$。
      3. 余弦相似度：$cos\theta = \frac{\vec{a} \cdot \vec{b}}{||\vec{a}||\times ||\vec{b}||}=\frac{\sum\limits_i x_i y_i}{\sqrt{\sum\limits_i x_i^2}\times \sqrt{\sum\limits_i y_i^2}}$。注意这里都是向量。
      4. 问题：为什么采用余弦相似度来表达特征的距离？余弦相似度相对于欧式距离由什么优势？
         1. **相似度应该和特征的大小无关，余弦相似度可以看成角度的距离。用余弦表示相似度相当于消除了向量模长对相似度的影响，反映到物品上表示相似度与物品的体量大小无关**。是不是余弦相似度还可以反映方向？
         2. 杰卡德相似度。
   ...
   8. 特征工程
      1. 连续值处理：归一化和离散化。
         1. 等步长离散化：这种分法不一定有效。因为等步长并不满足样本的分布，这个时候就需要进行等频率离散化。
         2. 等频率离散化：按照分布的样本数量再来划分。
         3. 两种离散化的优劣势对比
            1. 等频率的离散化方法精准，但是每次都需要对数据的分布进行计算，因为昨天用户的在电商上购买东西的价格分布和今天的不一定一样（这个地方感觉需要思考，确实每天的分布可能不一样，但是可以统计长时间的统计分布应该是一样的啊，是不是因为发现每天的购买商品的价格和时间的细粒度是相关的？很可能是这个原因）。昨天等频的切分点可能并不适用，而线上最需要避免的就是不固定，需要现场计算，所以训练出的模型今天不一定能使用。
            2. 等频不固定，但很精准；等步长固定，但是非常简单，因此在工业界上都有使用。
      2. 特征个数就是观测维度。
      3. 需要专业背景。
      4. 包括特征清洗、特征处理和特征选择。
      5. 不同特征的需要使用不同的处理方式。
         1. 数值型
         2. 类别型
            1. **要求将类别区分开，而且不能有预设的大小关系**。
            2. 常采用one-hot编码。
         3. 时间型。时间类型有是连续的情况也有是离散的情况。
            1. 连续值
               1. 持续时间。比如网页浏览的时长。
               2. 间隔时间。比如上一次购买/点击距离现在的时间间隔。
            2. 离散值
               1. 一天中哪个时间段
               2. 一周中的星期几
               3. 一年中那个月/星期
               4. 工作日/周末
         4. 统计型
            1. 加减平均。比如商品价格高于平均价格多少，用户在某个品类下消费超过多少。
            2. 分位线。商品属于售出商品价格的分位线。
            3. 次序线。商品处于热门商品中的第几位。
            4. 比例类。电商中商品的好/中/差评比例。
   9. 常见反馈数据
      |![常见反馈数据](../../pictures/RecommendationSystemCommonFeebackData.png "常见反馈数据")|
      |:--:|
      | *2.3* |
   10. 基于UGC的推荐。用户生成内容（标签）。（后面有部分重复了）
       1. 用户用标签来描述对物品的看法，所以用户生成标签（UGC）是联系用户和物品的纽带，也是反应用户兴趣的重要数据源。
       2. 一个用户标签行为的数据集一般由一个三元组（用户、物品、标签）的集合表示，其中一条记录(u,i,b)表示用户u给物品i打上了标签b。
       3. 如何衡量用户对一个打过标签的物品的喜爱程度呢？一个简单的算法：
          1. 统计每个用户最常用的标签
          2. 对于每个标签，统计被打过这个标签次数最多的物品。
          3. 对于一个用户，首先找到他最常用的标签，然后找到具有这个标签的的最热门的物品，推荐给他。
          4. 所以用户u对物品i的兴趣公式为$p(u,i)=\sum\limits_b n_{u,b}n_{b,i}\; \\ \text{其中,}n_{u,b}\text{表示用户u打过标签b的次数，}n_{b,i}\text{表示物品i被打过标签b的次数}$。$n_{u,b}$表示用户经常会使用这个标签，从另外一个角度说可以用这个标签作为用户自己的标签；$n_{b,i}$表示物品与标签b有多关联，同样的从另外一个角度说明这个标签可以作为这个物品的标签。
             1. 需要将$n_{u,b}$和$n_{b,i}$乘在一起用于表示两个变量之间的相关程度。两个变量的值越高，显然乘积的结果越大。两个变量之中的任何一个比较低，那么乘积的结果也比较小。
             2. 之所以要“求和”是一方面用户可能使用过很多标签，另一方面用户对这个物品可能打过很多标签。这就就把任何的多个标签都进行一次统计。总和的结果高，显然用户u对物品i更感兴趣。
             3. 这种算法的劣势在于：该算法倾向于对热门标签和热门物品比较大的权重。如果一个热门物品同时对应着热门标签，那么它就会霸榜，推荐的个性化、新颖度就会降低。一般的解决思路是要对热门物品和热门标签做一个惩罚。
4. 基于UGC的推荐
   1. 用户用标签来描述对物品的看法，所以用户生成标签(UGC)是联系用户和物品的纽带，也是反应用户兴趣的重要数据源。
...
5. TF-IDF Term Frequency-Inverse Document Frequency 词频-逆文档频率，是一种用于资讯检索和文本挖掘的常用加权技术。
   1. TF-IDF是一种统计方法，用以评估一个字词对于一个文件及或一个语料库中的其中一份文件的重要程度。**字词的重要性随着它在文件中出现的次数成正比增加，但同时会随着它在语料库中出现的频率成反比下降**。$TFIDF=TF \times IDF$。
   2. TF-IDF的主要思想是：如果某个词或短语在一篇文章中出现的频率（注意这里是频率，而不是频数。频数是词语出现的总词数，频率是频数除以总词数）TF高，并且在其他文章中很少出现，则认为此词或者短语具有很好的类别区分能力，适合用来分类。
   ...
   5. 逆向文件频率（Inverse Document Frequency, IDF）。是一个词语普遍重要性的度量，某一特定词语的IDF，可以由总文档数目除以包含该词语之文档的数目，再将得到的商取对数得到：$IDF_i = log(\frac{N+1}{N_i + 1})$。其中$IDF_i$表示词语i在文档集中的逆文档频率，N表示文档集中的文档总数，$N_i$表示文档集中包含了词语i的文档数。**另外需要+1的原因在于防止分母为0的情况出现。注意公式的形式，一般而言是总数是分母，出现的词数做分子，但是这个计算公式是出现该词的文档数在下，所以称其为“逆向”**。
   6. 基于TF-IDF思想对UGC进行了改进。为了避免热门标签和热门物品获得更多的权重，我们需要对“热门”进行惩罚。
      ...
      2. 由于物品i的所有标签$n_{*,j}$应该对标签权重没有影响，而“所有标签总数”N对于所有标签是一定的，所以这两项可以略去。在简单算法的基础上，直接加入对热门标签和热门物品的惩罚项：$p(u,i) = \sum\limits_{i}\frac{n_{u,b}}{log(1+n_b^{(u)})}\frac{n_{b,j}}{log(1+n_i^{(u)})}$。其中，$n_b^{(u)}$记录了标签b被多少个不同的用户使用过，表示的是热门标签。$n_i^{(u)}$记录了物品i被多少个不同的用户打过标签，表示的热门物品。注意相比于TF-IDF。分子没有总数项。这里是借鉴了TFIDF的思想做了一个设计。
   ...
   1. 基于近邻的推荐。原则是：相同“口碑”准则。
      ...
      1. 看行的维度，基于相似人员U(User CF)的。找两个用户对item评分的相似性。首先需要分析的是用户喜好的相似性，然后将相同喜好用户有偏好的物品推荐给新的用户。如图所示，问题：是否需要向Cary推荐泰坦尼克号？**首先观察偏好发现Alice和Cary的偏好类似（他们两人都对黑客帝国和虎胆龙威的评分比较低），然后观察Alice对于泰坦尼克号的评分，评分高那么就给予推荐，如果评分低那么就不推荐**。
         ...
         2. User-CF和基于人口统计学的推荐机制的异同：
            ...
            2. 不同点：首先是它们基于的数据不一样。人口统计信息是基于用户本身的数据。User-CF是基于行为数据。在真实对用户进行画像的时候。它们之间的不同点在于如何计算用户之间的相似度。基于人口统计学的机制只考虑用户本身的特征，而User-CF是在用户的历史偏好数据上计算用户的相似度，**它的基本假设是：喜欢类似物品的用户可能有相同的活着相似的口味和偏好**。
      ...
      2. 看列的维度，基于相似物品(Item CF)的。泰坦尼克号和那部电源的评分相近。从对比中可以看出泰坦尼克号和阿甘正传的评分类似（这种类似在于不同的人对两部电影的评分相近，比如Alice对泰坦尼克号和阿甘正传的评分都是5分。Bob对泰坦尼克号和阿甘正传的评分分别是1和2，都比较低。Diane对泰坦尼克号和阿甘正传的评分都是3分。而Cary对阿甘正传的评分是5分，那么可以推断Cary对泰坦尼克号的评分也会很高）。Item CF和User CF的基本原理类似。Item CF只是使用所有用户对物品的偏好，发现物品和物品之间的相似度，然后根据用户的历史偏好信息，将类似的物品推荐给用户。
         1. Item CF和基于内容(CB)的推荐之间有什么异同？
            ...
            2. 不同点：首先是计算的数据不一样。CB是基于物品本身的信息。Item-CF是基于的行为数据。计算相似度的方法不一样。**Item CF是从用户的历史偏好来推断，而CB是基于物品本身的属性特征信息进行推荐**。
      3. 同样是CF，在基于user和基于item两个吃了中应该如何选择？**比较用户和物品在这个应用场景下，那个存在得更稳定**。
         ...
         3. 推荐策略的选择其实和具体的应用场景有非常大的关系。
   2. 基于模型的协同过滤思想
      1. 数据源依然是行为数据。
      2. 基本思想
         1. 用户具有一定的特征，决定着他的偏好选择。
         2. 物品具有一定的特征，影响着用户是否选择它。
         3. 用户之所以选择某一个物品是因为用户偏好特征与物品的特征相互匹配。
      3. 基于这种思想，模型的建立相当于从行为数据中提取特征，给用户和物品同时打上标签，这和基于人口统计学的用户标签、基于内容方法的物品标签本质是一样的，都是特征的提取和匹配。
      4. 有显性特征时（比如用户标签、物品分类标签）我们可以直接匹配做出推荐；没有时，可以根据已有的偏好数据去发掘隐藏的特征。这需要用到隐语义模型（LFM）。
      5. 基于模型的协同过滤推荐就是基于样本的用户偏好信息，训练一个推荐模型，然后根据实时的用户喜好信息进行预测新物品的得分，从而计算推荐。
      6. 比较基于近邻的推荐和基于模型的推荐
         1. 基于近邻的推荐是在预测时直接使用已有的用户偏好数据，通过近邻数据来预测对新物品的偏好（类似分类）。
         2. 基于模型的推荐的方法是要使用这些偏好数据来训练模型，找到内在规律，再用模型来做预测（类似回归）。
      7. 训练模型时，可以基于标签内容来提取物品特征，也可以让模型去发掘物品的潜在特征；这样的模型被称为隐语义模型（Latent Factor Model, LFM）。
   3. LFM
      1. 用隐语义模型来进行协同过滤的目标
         1. 揭示隐藏的特征，这些特征能够解释为什么给对应的预测评分。
         2. 这类特征可能是无法直接用语言解释描述的，事实上我们并不需要知道，类似的玄学。（理解：实际上是将不同维度的特征映射到了一个相同的特征空间，然后在同一个特征空间之后就可以进行各种运算了）。
      2. 通过矩阵分解及进行降维分析
         1. 协同过滤算法非常依赖历史数据，而一般的推荐系统中，偏好数据又往往是稀疏的；这就需要对原始数据进行降维处理。降维处理既是手段又是目的。
         2. 分解之后的矩阵，就代表了用户和物品的隐藏特征。就是将一个比较大的矩阵分解为两个小矩阵相乘，分解之后得到的小矩阵就可以代表隐藏特征。
      3. 隐语义模型的具体实例
         1. 基于概率的隐语义分析（pLSA）
         2. 隐式狄利克雷分布模型（LDA）
         3. 矩阵因子分解模型（基于奇异值分解的模型，SVD）
   4. **LFM降维方法-矩阵因子分解**。这个部分最重要。[参考实现代码](../../codes/4BasicKnowledgePoints/6GradientDescentMethod.ipynb)
      1. 目的：将行为数据矩阵转换为两个小矩阵的乘积。
      2. 通过**交叉验证**来调整正则化系数。
      3. 输入：假设用户物品的评分矩阵为$\boldsymbol{R}$，现在有m个用户和n个物品。
      4. 目的：希望发现k维的隐性空间。也就是找到两个矩阵$\boldsymbol{P}$和$\boldsymbol{Q}$，使得两个矩阵的乘积近似等于$\boldsymbol{R}$
      5. 在矩阵因式分解的过程中，为了是得目标函数求得极值，需要将其导数等于0，来找到极值点。
      6. 为了解决在矩阵分解中两个（或者多个矩阵）同时变化的情况，这里使用的是交替最小二乘法来完成梯度下降的计算。交替最小二乘法ALS的核心思想是：
         1. 从多个变量，选定一个变量；
      ...
      7. 这里为什么要用梯度下降来求矩阵分解？是因为在6中需要求矩阵的逆。一般认为矩阵的逆是不好计算的。所以通过梯度下降来迭代求出矩阵的分解因子。这样就不需要目标函数的导数等于0了，只需要求偏导数乘以步长，来进行迭代即可。
7. 推荐系统实践
   1. 组成部分
      1. 项目框架
      2. 数据源分析
   ...
   4. 系统模块化设计
      ...
      3. 推荐方式的划分
         ...
         3. 基于内容的推荐
      4. 业务显示页面
   5. 一般系统架构及相关使用的具体工具。![一般推荐系统架构图](../../pictures/GeneralRecommendationSystemArchitecture.png "一般推荐系统架构图")
      1. 用户可视化：AngularJS
      2. 综合业务服务：apache和spring
      3. 业务数据库：mongoDB
      4. 搜索服务器：ElasticSearch
      5. 缓存数据库：redis
      6. 离线统计服务：Spark SQL，结果会写入mongoDB
      7. 离线推荐服务：Spark MLlib，结果会写入mongoDB
      8. 工作调度服务：Azkaban。它将离线统计服务和离线推荐服务调度起来定时运行，然后定时更新数据库中的数据。
      9. 日志采集服务：Flume-ng，从spring中获取日志信息，传递给消息缓冲服务。
      10. 消息缓冲服务：Kafka，将缓冲的消息传递给实时推荐服务。
      11. 实时推荐服务：Spark streaming，从消息缓冲服务和缓存数据库中获取信息，最终得到结果。将结果传递给业务数据库。
   6. 离线推荐模块
      1. 用ALS（Alternating Least Square，交替最小二乘法）算法训练隐语义模型
         1. 由于是评分，是连续值。可以使用RMSE均方根误差作为目标函数。
      2. 计算用户推荐矩阵
      3. 计算电源相似度矩阵
         1. 基于行为数据的方法（也就是协同过滤）是：通过对评分矩阵进行矩阵因式分解之后得到的电影特征矩阵来计算电源之间的相似度。
         2. 基于内容数据的方法（也就是基于内容的推荐）是：直接通过电源的分类标签来对电源的相似度进行计算。
   7. 实时推荐模块框架P41
      1. 要求：
         1. 计算速度要快
         2. 结果可以不是特别精确
      ...
      2. 输入
         1. 用户对当前电影的评分
         2. 所有电影的相似度矩阵
      3. 为了将评分也考虑到推荐的因素里面，需要将评分和相似度进行加权。具体执行如下：
         1. 首先将某一电影与研究对象的电影计算相似度；
         2. 将某一电影的评分与第一步中得到的相似度进行乘积计算；
         3. 然后对多个电影（最近k次评分的电影）与研究对象电影重复1、2步骤，并对结果求和。
         4. 将3中的结果除以多个电影的数量。
         5. 定义惩罚项和奖励项。
            1. 对多次评分高的电影做奖励。$\lg max{incount,1}$，其中Incount是高分的电影个数，之所以需要和1之间求max，是担心高分电影的个数为0。这里可以人工定义高分电影的判断标准，比如可以定义大于等于3分的为高分电影。
            2. 对多次评分顶的电影做惩罚。$-\lg max{incount,1}$，**注意前面有负号**。其中Incount是低分的电影个数，之所以需要和1之间求max，是担心低分电影的个数为0。这里可以人工定义低分电影的判断标准，比如可以定义小于3分的为高分电影。
